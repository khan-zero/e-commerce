...
from .models import (
    ...
    Review, Like, Comment # Import new models
)
from .serializers import (
    ...
    UserRegistrationSerializer, # Import registration serializer
    ReviewSerializer, LikeSerializer, CommentSerializer # Import new serializers
)
...
class CartViewSet(viewsets.ModelViewSet):
    ...
    def checkout(self, request):
        ...
        shipping_address_line1 = request.data.get('shipping_address_line1', user.address)
        ...
        with transaction.atomic():
            ...
            for cart_item in cart.items.all():
                ...
                OrderItem.objects.create(
                    order=order,
                    product=product,
                    price=product.price, # Capture current product price at checkout
                    quantity=quantity
                )
                product.stock -= quantity # Reduce stock
                product.save()
...
# --- NEW: Review ViewSet (from JS review.routes.js & review.controller.js) ---
class ReviewViewSet(viewsets.ModelViewSet):
    ...
    def get_permissions(self):
        if self.action == 'create':
            return [permissions.IsAuthenticated()] # Only authenticated users (buyers) can create reviews
        if self.action in ['destroy']:
            return [IsOwnerOrAdmin()] # Only the review owner or admin can delete
        return [permissions.AllowAny()] # Anyone can list or retrieve reviews

    def perform_create(self, serializer):
        # Ensure the review is for the authenticated user and a valid product
        product_id = self.request.data.get('product')
        if not product_id:
            raise ValidationError({'product': 'Product ID is required.'})
...
# --- NEW: Like ViewSet (from JS like.routes.js & like.controller.js) ---
class LikeViewSet(viewsets.ModelViewSet):
    queryset = Like.objects.all().select_related('product', 'user')
    serializer_class = LikeSerializer
    permission_classes = [permissions.IsAuthenticated] # Default for likes (add/remove/status)
    ...
    def get_permissions(self):
        ...
        return [permissions.IsAdminUser()] # Default for list/retrieve/destroy (admin only for general management)

    def perform_create(self, serializer):
        # The serializer's create method already handles duplicate likes, but we can add product validation here
        product_id = self.request.data.get('product')
        if not product_id:
            raise ValidationError({'product': 'Product ID is required.'})
        try:
...
# --- NEW: Comment ViewSet (from JS comment.routes.js & comment.controller.js) ---
class CommentViewSet(viewsets.ModelViewSet):
    ...
    def get_permissions(self):
        if self.action == 'create':
            return [permissions.IsAuthenticated()] # Only authenticated users can create comments
        if self.action == 'destroy':
            return [IsOwnerOrAdmin()] # Only the comment owner or admin can delete
        return [permissions.AllowAny()] # Anyone can list or retrieve comments

    def perform_create(self, serializer):
        # Ensure the comment is for the authenticated user and a valid product
        product_id = self.request.data.get('product')
...
# --- NEW: Analytics ViewSet (from JS analytics.routes.js) ---
